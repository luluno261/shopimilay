...
# Version améliorée avec support pnpm
...
if ($dockerInstalled) {
    ...
    Write-Host "⚠️  Docker n'est pas installé ou pas dans le PATH" -ForegroundColor Yellow
    Write-Host "   Installez Docker depuis https://www.docker.com/" -ForegroundColor Yellow
}
...
if ($usePnpm) {
    ...
    Write-Host "❌ Ni pnpm ni npm ne sont installés." -ForegroundColor Red
    exit 1
}
...
"@
    $content | Out-File -FilePath "apps\api-gateway\.env.example" -Encoding UTF8 -NoNewline
...
"@
    $content | Out-File -FilePath "apps\marketing-engine\.env.example" -Encoding UTF8 -NoNewline
    Write-Host "  ✅ Créé apps\marketing-engine\.env.example" -ForegroundColor Green
}

# Admin Dashboard
if (-not (Test-Path "web\admin-dashboard\.env.example")) {
    "NEXT_PUBLIC_API_URL=http://localhost:8080/api/v1" | Out-File -FilePath "web\admin-dashboard\.env.example" -Encoding UTF8 -NoNewline
...
if (-not (Test-Path "web\storefront-template\.env.example")) {
    "NEXT_PUBLIC_API_URL=http://localhost:8080/api/v1" | Out-File -FilePath "web\storefront-template\.env.example" -Encoding UTF8 -NoNewline
    Write-Host "  ✅ Créé web\storefront-template\.env.example" -ForegroundColor Green
}

# Copier les .env.example vers .env.local pour Next.js (si n'existent pas)
...
Write-Host "  1. Démarrer l'infrastructure : make infra-up (ou docker-compose up -d)" -ForegroundColor White
...
Write-Host "     - Admin Dashboard: cd web\admin-dashboard ; $packageManager run dev" -ForegroundColor White
Write-Host "     - Storefront: cd web\storefront-template ; $packageManager run dev" -ForegroundColor White
Write-Host ""
